...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        # DataModel.objects.filter(filed_1=value_1, filed_2=value_2, filed=3)
        ...
        # Find subject id from subjects title
        for i, title in subjects_context.items():
            if title == subjects_title:
                subject_id = i
                break
        
        # Find credit id from subject id
        for i, credit, subject_id_list_that_using_this_credit in credits_context.items():
            if subject_id in subject_id_list_that_using_this_credit:
                subject_credit = credit

        # # Create Objects Example
        # DataModel.objects.create(filed_1=value_1, filed_2=value_2, filed_3=value_3)
        student = Personnel.objects.get(first_name=student_first_name)
        subject = Subjects.objects.get(title=subjects_title)
        student_score_subject = StudentSubjectsScore.objects.filter(student=student, subject=subject).first()

        if student_score_subject:
            student_score_subject.score = score
            student_score_subject.save()

        else:
            StudentSubjectsScore.objects.create(student=student, subject=subject, credit=subject_credit, score=score)
